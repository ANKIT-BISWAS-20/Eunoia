...
export default function LeaderTaskList({ teamTasks }) {
  ...
  const markTask = async (e, markingSubmissionId) => {
    e.preventDefault();
    try {
  ...
  const deleteTask = async (e) => {
    ...
    try {
      ...
      const response = await axios.delete(`${Backend_url}/api/v1/tasks/delete-task?teamId=${teamId}&taskId=${selectedTask._id}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
  ...
  const getSubmissionsInfo = async () => {
    try {
      ...
      const response = await axios.get(`${Backend_url}/api/v1/submissions/view-all-submissions?taskId=${selectedTask._id}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
                  ...
                  {() => (
                    <>
                      <ModalHeader className="flex flex-col gap-1">
                        {
                          key === "View" && (
                            <h1 className="text-2xl font-bold font-inter">View Task</h1>
                          )
                        }
                        {
                          key === "View Submissions" && (
                        ...
                        {
                          key === "delete" && (
                            <h1 className="text-2xl font-bold font-inter">Delete Task</h1>
                          )
                        }
                                ...
                                {allSubmissions?.map((submission, index) => (
                                  <AccordionItem key={index} aria-label={`Accordion ${index + 1}`} title={submission.name} className="font-roboto text-xl">
                                    ...
                                    {submission.marks === "unmarked" && (<>
                                      ...
                                      <Button color="success" variant="solid" onClick={(e) => markTask(e, submission.id)}>Mark</Button>
                                      </div>
                                    </>)}
                        ...
                        }
                        {
                          key === "delete" && (
                            <div className="items-center text-center">
                              <h1 className="my-8">Are you sure you want to delete this task?</h1>
                              <Button color="danger" variant="solid" onClick={(e) => deleteTask(e)}>
                                Delete
                              </Button>
                            </div>
                          )
